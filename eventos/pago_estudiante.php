...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'agregar_pago') {
        ...
        if (!$errores) {
            editarPagoEstudiante(db(), 'evento', $id, $estudianteId, $pagoId, $monto, $metodo, $fechaPago, $nota);
            flash('Pago actualizado.');
            redirect('pago_estudiante.php?id=' . $id . '&estudiante_id=' . $estudianteId);
        } else {
            // Si la corrección no pasa validación, se vuelve a abrir en modo
            // edición (en vez de perder el modo y caer al formulario de
            // "Agregar pago" vacío) para que la persona corrija sin tener
            // que volver a hacer clic en "Editar".
